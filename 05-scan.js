...
const Scanner = {
  ...
  loop(video, run){
    const REST = 1;        /* rest for 1x the time the last read took */
    const FLOOR = 40;      /* never read more than ~25 times a second */
    let next = 0;
    const step = () => {
      if (run !== this.run) return;
      ...
      if (now < next) return;
      if (this.paused()){ next = now + 90; return; }
      this.frame++;
      const w = 480, h = Math.round(video.videoHeight / video.videoWidth * w) || 480;
      this.cv.width = w; this.cv.height = h;
      this.ctx.drawImage(video, 0, 0, w, h);
      let img;
      try { img = this.ctx.getImageData(0, 0, w, h); } catch (_) { next = now + FLOOR; return; }
      const hit = jsQR(img.data, w, h, { inversionAttempts:'dontInvert' });
      next = performance.now() + Math.max(FLOOR, (performance.now() - now) * REST);
      if (hit && hit.data){
        this.locked = true;
