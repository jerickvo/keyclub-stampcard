...
test('a station reads reached exactly when it is: sealed with the stamps still to come, far beyond the next, claimed once', () => {
  ...
  const scans = n => Array.from({ length:n }, (_, i) => ({ meetingId:'m' + i, at:'2026-09-01T00:00:00Z' }));
  const says = html => (html.match(/tier__say">([^<]*)</) || [])[1];

  S.scans = scans(9); S.rewards = T.map(r => ({ ...r, claimed:false }));
  const nine10 = C.tier(S.rewards[0], 9, 0), nine20 = C.tier(S.rewards[1], 9, 10);
  assert.equal(nine10.includes('tier--sealed'), true);
  assert.equal(nine10.includes('1 more stamp<'), true);
  assert.equal(nine20.includes('tier--far'), true);             // nothing toward 20 before 10 is reached
  assert.equal(says(nine20), undefined);                          // and nothing counted toward it
  assert.equal(nine10.includes('data-claim'), false);

  S.scans = scans(13); S.rewards = T.map(r => ({ ...r, claimed:r.id === 'r1' }));
  const t10 = C.tier(S.rewards[0], 13, 0), t20 = C.tier(S.rewards[1], 13, 10);
  assert.equal(says(t10), undefined);                             // a claimed station is done: nothing still to come
  ...
  assert.equal(ready.includes('tier__word">Claim<'), true);       // the claim is printed inside the stamp
});
